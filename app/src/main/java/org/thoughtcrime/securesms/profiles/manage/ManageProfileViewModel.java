      ...
      repository.clearAvatar(context, result -> {
        switch (result) {
          case SUCCESS:
            ...
            previousAvatar = null;
            repository.profileJavaPresenter.getValue().repository.updateAvatar(null);
            break;
          ...
          repository.setAvatar(context, data, media.getMimeType(), result -> {
            switch (result) {
              case SUCCESS:
                ...
                previousAvatar = data;
                repository.profileJavaPresenter.getValue().repository.updateAvatar(data);
                break;
              case FAILURE_NETWORK:
                internalAvatarState.postValue(InternalAvatarState.loaded(previousAvatar));
                events.postValue(Event.AVATAR_NETWORK_FAILURE);
...
//    username.postValue(recipient.getUsername().orElse(null));
    repository.profileJavaPresenter.getValue().repository.getUsernameAsync(u->{
      username.postValue(u);
      return null;
    });
