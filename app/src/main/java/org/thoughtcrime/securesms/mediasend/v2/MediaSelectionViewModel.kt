...
class MediaSelectionViewModel(
  ...
  fun setSelectedMedia(medias: List<Media>){
    store.update {
      it.copy(selectedMedia = medias)
    }
  }
  ...
  fun isAddingNewPost(): Boolean {
    return store.state.isAddingNewPost
  }
  ...
  public fun addMedia(media: List<Media>) {
  ...
  fun transformMedia(): Maybe<List<Media>> {
    return repository.transformMedia(
                    store.state.selectedMedia,
                    store.state.editorStateMap
    )
  }
  ...
  class Factory(
    ...
    private val isAddingNewPost: Boolean,
    private val repository: MediaSelectionRepository
  ) : ViewModelProvider.Factory {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
      return requireNotNull(modelClass.cast(MediaSelectionViewModel(destination, sendType, initialMedia, initialMessage, isReply, isStory, isAddingNewPost, repository)))
    }
  }
}
