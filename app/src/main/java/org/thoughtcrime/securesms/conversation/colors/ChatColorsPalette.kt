...
object ChatColorsPalette {
  object Bubbles {
    ...
    @JvmField
    var ULTRAMARINE = getForColor()
    fun getForColor():ChatColors{
      return ChatColors.forColor(
              ChatColors.Id.BuiltIn,
              getCustomChatColor())
    }
    fun getCustomChatColor():Int{
      return if(SignalStore.settings().theme.toString()=="SYSTEM"){
        if(isDarkMode(ApplicationContext.applicationContext)){
          0xff084037.toInt()
        }else{
          0xffe2ffd3.toInt()
        }
      }else if(SignalStore.settings().theme.toString()=="DARK"){
        0xff084037.toInt()
      }else{
        0xffe2ffd3.toInt()
      }
    ...
    private fun isDarkMode(context: Context): Boolean {
      val darkModeFlag = context.resources.configuration.uiMode and
