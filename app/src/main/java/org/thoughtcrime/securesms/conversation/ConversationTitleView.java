...
import org.linkmessenger.utils.ContextUtils;
...
public class ConversationTitleView extends RelativeLayout {
  ...
  public void setTitle(@NonNull GlideRequests glideRequests, @Nullable Recipient recipient) {
    ...
    if (recipient != null) {
      if(recipient.showVerified()){
        endDrawable = ContextUtils.INSTANCE.getUserBadge(getContext(),3, 24);
      }else {
        endDrawable = ContextUtils.INSTANCE.getUserBadge(getContext(), recipient.getBadgeId(getContext()), 24);
      }
    }
  ...
  public void setOnStoryRingClickListener(@NonNull OnClickListener onStoryRingClickListener, OnClickListener onAvatarClickListener) {
...
//    this.verified.setVisibility(verified ? View.VISIBLE : View.GONE);
  ...
  private void setIndividualRecipientTitle(@NonNull Recipient recipient) {
    ...
    if (recipient.getOnlineStatusItem()!=null && recipient.getOnlineStatusItem().getLastEntranceTimestamp()>0){
      try {
        long timeDifference = Math.abs(new Date().getTime() - recipient.getOnlineStatusItem().getLastEntranceTimestamp());
        if (timeDifference <= 60000) {
          this.subtitle.setText(R.string.online);
        }else {
          this.subtitle.setText(getContext().getString(R.string.last_online) + " " + DateUtils.getAgoTimeSpanString(getContext(), Locale.getDefault(), recipient.getOnlineStatusItem().getLastEntranceTimestamp()));
        }
