...
import org.whispersystems.signalservice.internal.push.OnlineStatusRequest;
...
public class ConversationViewModel extends ViewModel {
  ...
  private final MutableLiveData<OnlineStatusResponse>     usersOnlineStatus;
  ...
  private ConversationViewModel() {
    ...
    this.usersOnlineStatus                   = new MutableLiveData<>();
  ...
  public void loadUserOnlineStatus(List<String> accounts) {
    if(accounts.isEmpty()){
      return;
    }
    disposables.add(
            Observable.fromCallable(() -> ApplicationDependencies.getSignalServiceAccountManager().getUsersOnlineStatus(new OnlineStatusRequest(accounts)))
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribe(this::setUsersStatus, (Throwable throwable) -> {
            Log.e("load online status", throwable);
        })
    );
  }
  private void setUsersStatus(OnlineStatusResponse onlineStatusResponse) {
    usersOnlineStatus.postValue(onlineStatusResponse);
  }

  public MutableLiveData<OnlineStatusResponse> getUsersOnlineStatus() {
