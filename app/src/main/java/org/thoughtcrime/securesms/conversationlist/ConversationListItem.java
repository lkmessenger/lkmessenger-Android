...
public final class ConversationListItem extends ConstraintLayout implements BindableConversationListItem, Unbindable {
  ...
  private Set<Long>           onlineThreads;
  ...
  {
    bindThread(lifecycleOwner, thread, glideRequests, locale, typingThreads, onlineThreads, verifiedThreads, selectedConversations, null);
  }
                         ...
                         @Nullable String highlightSubstring)
  {
    ...
    this.onlineThreads = onlineThreads;
    updateOnlineIndicator(onlineThreads);

    updateVerifiedIndicator(verifiedThreads);
  ...
  private void setBadgeFromRecipient(Recipient recipient) {
    if (!recipient.isSelf()) {
      if (recipient.getOnlineStatusItem()!=null){
        long timeDifference = Math.abs(new Date().getTime() - recipient.getOnlineStatusItem().getLastEntranceTimestamp());
        if (timeDifference <= 60000) {
          badge.setImageDrawable(ContextCompat.getDrawable(getContext(), R.drawable.online_status));
          badge.setScaleType(ImageView.ScaleType.CENTER);
          badge.setClickable(false);
        }
      }
//      badge.setBadgeFromRecipient(recipient);
//      badge.setImageDrawable(ContextCompat.getDrawable(getContext(), R.drawable.ic_badge_24));
//      badge.setClickable(false);
    } else {
      badge.setBadge(null);
    }
  ...
  @Override
  public void updateOnlineIndicator(@NonNull Set<Long> onlineThreads) {
    if(onlineThreads.contains(threadId)){
      badge.setImageDrawable(ContextCompat.getDrawable(getContext(), R.drawable.online_status));
      badge.setScaleType(ImageView.ScaleType.CENTER);
      badge.setClickable(false);
    }else {
  ...
  @Override
  public void updateVerifiedIndicator(@NonNull Map<Long, Integer> verifiedThreads) {
//    Integer status = verifiedThreads.get(threadId);
//    if(status!=null){
//
//    }
  }
