...
class ConversationListAdapter extends ListAdapter<Conversation, RecyclerView.ViewHolder> {
  ...
  private static final int TYPE_STORIES      = 5;
  ...
  private final Set<Long>                   onlineSet             = new HashSet<>();
  private final Map<Long, Integer>                   verifiedSet             = new HashMap<>();
      ...
      for (Object payloadObject : payloads) {
        if (payloadObject instanceof Payload) {
          ...
          if (payload == Payload.SELECTION) {
            ...
            ((ConversationViewHolder) holder).getConversationListItem().updateOnlineIndicator(onlineSet);
          }if(payload == Payload.VERIFIED){
            ((ConversationViewHolder) holder).getConversationListItem().updateVerifiedIndicator(verifiedSet);
          }else {
  ...
  void setOnlineThreads(@NonNull Set<Long> onlineThreadSet) {
    this.onlineSet.clear();
    this.onlineSet.addAll(onlineThreadSet);

    notifyItemRangeChanged(0, getItemCount(), Payload.ONLINE);
  }
  void setVerifiedThreads(@NonNull Map<Long, Integer> verifiedThreadSet) {
    this.verifiedSet.clear();
    this.verifiedSet.putAll(verifiedThreadSet);

    notifyItemRangeChanged(0, getItemCount(), Payload.VERIFIED);
  }
