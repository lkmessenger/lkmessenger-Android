...
import org.thoughtcrime.securesms.stories.landing.StoriesLandingItemData
 ...
 */
object PostsStoryLandingItem {

  private const val STATUS_CHANGE = 0

  fun register(mappingAdapter: MappingAdapter) {
    mappingAdapter.registerFactory(Model::class.java, LayoutFactory(::ViewHolder, R.layout.posts_landing_item))
  }

  class Model(
          val data: StoriesLandingItemData,
          val onAvatarClick: (Model) -> Unit,
...
//      }
      ...
      sender.text = when {
        ...
        else -> getUserPresentation(model)
      }
...
//      }
    ...
    private fun setUpClickListeners(model: Model) {
      ...
      if (model.data.storyRecipient.isMyStory) {
        ...
        addToStoriesView.visibility = View.VISIBLE

        addToStoriesView.setOnClickListener {
          model.onAddToStoryClick()
        }
      } else {
        ...
        addToStoriesView.visibility = View.INVISIBLE
      }
    }
      private fun getUserPresentation(model: Model): CharSequence {
          return if(model.data.storyRecipient.e164.isPresent && model.data.storyRecipient.e164.get().startsWith("+1777")){
    ...
    private fun displayContext(model: Model) {
      ...
      PostContextMenu.show(context, itemView, storyPreview,model) { itemView.isSelected = false }
    }
//
