...
//  private View                    serviceWarning;
  ...
  private MaterialButton          didntReceiveBtn;
  ...
  public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
    ...
    didntReceiveBtn = view.findViewById(R.id.not_received_otp);
//    serviceWarning       = view.findViewById(R.id.cell_service_warning);
    ...
    didntReceiveBtn.setOnClickListener(new View.OnClickListener() {
      @Override
    ...
    viewModel.getSuccessfulCodeRequestAttempts().observe(getViewLifecycleOwner(), (attempts) -> {
      if (attempts >= 3) {
        didntReceiveBtn.setVisibility(View.VISIBLE);
...
//    if(viewModel.getNumber().getCountryCode()==7 || viewModel.getNumber().getCountryCode()==375 || viewModel.getNumber().getCountryCode()==998){
...
//    }
  ...
  private void setOnCodeFullyEnteredListener(VerificationCodeView verificationCodeView) {
    verificationCodeView.setOnCompleteListener(code -> {
      ...
      didntReceiveBtn.setVisibility(View.INVISIBLE);
  ...
  private void handlePhoneCallRequestAfterConfirm() {
    Disposable request = viewModel.requestVerificationCode(VerifyAccountRepository.Mode.SMS_WITH_LISTENER)
                                  .observeOn(AndroidSchedulers.mainThread())
                                  .subscribe(processor -> {
                                    if (processor.hasResult()) {
