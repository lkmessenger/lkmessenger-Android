...
import org.linkmessenger.profile.view.activities.ProfileActivity.Companion.newIntent
...
class ConversationSettingsFragment : DSLSettingsFragment(
  ...
  private val blockIcon by lazy {
    ContextUtil.requireDrawable(requireContext(), R.drawable.ic_fluent_presence_blocked_24_regular).apply {
      colorFilter = PorterDuffColorFilter(alertTint, PorterDuff.Mode.SRC_IN)
    }
  }

  private val unblockIcon by lazy {
    ContextUtil.requireDrawable(requireContext(), R.drawable.ic_fluent_presence_blocked_24_regular)
  }

  private val leaveIcon by lazy {
    ContextUtil.requireDrawable(requireContext(), R.drawable.ic_fluent_arrow_exit_20_regular).apply {
      colorFilter = PorterDuffColorFilter(alertTint, PorterDuff.Mode.SRC_IN)
    }
  }
...
//      }
      ...
      if (!state.recipient.isGroup){
        clickFirstPref(
          title = DSLSettingsText.from(R.string.openprofile),
          icon = DSLSettingsIcon.from(R.drawable.ic_fluent_open_24_regular, R.color.signal_colorPrimary),
          onClick = {
            try {
              val intent = newIntent(requireActivity(), null, null, state.recipient.serviceId.get().uuid().toString())
              startActivity(intent)
            }catch (_:Exception){
      ...
      val icon = if (state.disappearingMessagesLifespan <= 0 || state.recipient.isBlocked) {
        R.drawable.ic_fluent_timer_off_24_regular
      } else {
        R.drawable.ic_fluent_timer_24_regular
      }
      ...
      if (!state.recipient.isReleaseNotes) {
        clickPref(
          ...
          icon = DSLSettingsIcon.from(R.drawable.ic_fluent_color_24_regular),
          onClick = {
      ...
      if (!state.recipient.isSelf) {
        clickPref(
                ...
                icon = DSLSettingsIcon.from(R.drawable.ic_fluent_speaker_2_24_regular),
                onClick = {
      ...
      state.withRecipientSettingsState {
        ...
        when (recipientState.contactLinkState) {
          ...
          ContactLinkState.ADD -> {
            ...
            clickPref(
              ...
              icon = DSLSettingsIcon.from(R.drawable.ic_fluent_add_24_regular),
              onClick = {
                try {
        ...
        if (recipientState.identityRecord != null) {
          clickLastPref(
            ...
            icon = DSLSettingsIcon.from(R.drawable.ic_fluent_shield_checkmark_24_regular),
            onClick = {
        ...
        if (recipientSettingsState.selfHasGroups && !state.recipient.isReleaseNotes) {
          ...
          if (!state.recipient.isBlocked) {
            clickSinglePref(
                    ...
                    icon = DSLSettingsIcon.from(R.drawable.ic_fluent_add_circle_24_filled),
                    onClick = {
          ...
          if (recipientSettingsState.canShowMoreGroupsInCommon) {
            customPref(
              LargeIconClickPreference.Model(
                title = DSLSettingsText.from(R.string.ConversationSettingsFragment__see_all),
                icon = DSLSettingsIcon.from(R.drawable.ic_fluent_chevron_circle_down_24_filled, NO_TINT),
                onClick = {
      ...
      state.withGroupSettingsState { groupState ->
        ...
        if (groupState.canAddToGroup) {
          customPref(
            LargeIconClickPreference.Model(
              ...
              icon = DSLSettingsIcon.from(R.drawable.ic_fluent_add_circle_24_filled, NO_TINT),
              onClick = {
                viewModel.onAddToGroup()
              }
            )
        ...
        if (state.recipient.isPushV2Group) {
          ...
          clickFirstPref(
            ...
            icon = DSLSettingsIcon.from(R.drawable.ic_fluent_link_24_regular),
            onClick = {
          ...
          if (groupState.isSelfAdmin) {
            clickPref(
                    ...
                    icon = DSLSettingsIcon.from(R.drawable.ic_fluent_person_add_24_regular),
                    onClick = {
