...
class NotificationsSettingsFragment : DSLSettingsFragment(R.string.preferences__notifications) {
  ...
  override fun bindAdapter(adapter: MappingAdapter) {
    ...
    val factory = NotificationsSettingsViewModel.Factory(sharedPreferences, profileRepository)
  ...
  private fun getConfiguration(state: NotificationsSettingsState): DSLConfiguration {
    return configure {
      ...
      sectionHeaderPref(R.string.Posts)

      switchFirstPref(
        title = DSLSettingsText.from(R.string.new_follower_notification),
        isChecked = state.postNotificationsState.newFollowerNotificationEnabled,
        onClick = {
          viewModel.setNewFollowerNotificationEnabled(!state.postNotificationsState.newFollowerNotificationEnabled)
        }
      )

      switchPref(
        title = DSLSettingsText.from(R.string.post_liked_notification),
        isChecked = state.postNotificationsState.likeNotificationEnabled,
        onClick = {
          viewModel.setLikeNotificationEnabled(!state.postNotificationsState.likeNotificationEnabled)
        }
      )

      switchLastPref(
        title = DSLSettingsText.from(R.string.post_commented_notification),
        isChecked = state.postNotificationsState.postCommentedNotification,
        onClick = {
          viewModel.setPostCommentedNotification(!state.postNotificationsState.postCommentedNotification)
        }
      )
