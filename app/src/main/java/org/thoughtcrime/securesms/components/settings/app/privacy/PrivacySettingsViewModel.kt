...
class PrivacySettingsViewModel(
  ...
  fun setShowPhoneNumber(enabled: Boolean) {
    sharedPreferences.edit().putBoolean(TextSecurePreferences.SHOW_PHONE_NUMBER, enabled).apply()
    repository.syncReadReceiptState()
    refresh()
  }

  fun setMessengerAccess(enabled: Boolean) {
    sharedPreferences.edit().putBoolean(TextSecurePreferences.MESSENGER_ACCESS, enabled).apply()
    repository.syncReadReceiptState()
    refresh()
  }
  fun setOnlinePrivacy(enabled: Boolean) {
    repository.setOnlineStatusPrivacy(enabled){
      if(!it){
        sharedPreferences.edit().putBoolean(TextSecurePreferences.ONLINE_STATUS_PRIVACY, !enabled).apply()
        repository.syncReadReceiptState()
        refresh()
      }
    }
  ...
  fun setOnlineStatusPrivacy(onlineStatusPrivacy: Boolean){
  ...
  private fun getState(): PrivacySettingsState {
    return PrivacySettingsState(
      ...
      showPhoneNumber = TextSecurePreferences.isShowPhoneNumberEnabled(ApplicationDependencies.getApplication()),
      messengerAccess = TextSecurePreferences.isMessengerAccessEnabled(ApplicationDependencies.getApplication()),
      onlineStatusPrivacy = TextSecurePreferences.getOnlineStatusPrivacy(ApplicationDependencies.getApplication())
    )
  }
