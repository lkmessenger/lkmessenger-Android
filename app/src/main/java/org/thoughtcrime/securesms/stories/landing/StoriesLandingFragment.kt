...
class StoriesLandingFragment : DSLSettingsFragment(layoutId = R.layout.stories_landing_fragment) {
  ...
  private var toolbar: Stub<Toolbar>? = null
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    toolbar!!.get().setNavigationOnClickListener { v: View? -> NavHostFragment.findNavController(this).popBackStack() }
    toolbar!!.get().setTitle(R.string.all_stories)
    ...
    setEnterSharedElementCallback(object : SharedElementCallback() {
      override fun onSharedElementStart(sharedElementNames: MutableList<String>?, sharedElements: MutableList<View>?, sharedElementSnapshots: MutableList<View>?) {
        if (sharedElementNames?.contains("camera_fab") == true) {
          cameraFab.setImageResource(R.drawable.ic_add_story_24_regular)
  ...
  interface Callback : Material3OnScrollHelperBinder {
    fun getBasicToolbar(): Stub<Toolbar>
  }
}
