...
internal class AccountValues internal constructor(store: KeyValueStore) : SignalStoreValues(store) {
  ...
  companion object {
    ...
    private const val KEY_SOCIAL_TOKEN = "account.social_token"
    private const val KEY_SOCIAL_REGISTRED = "account.social_registred"
  ...
  var socialToken: String?
    get() {
      return getString(KEY_SOCIAL_TOKEN, null)
    }
    set(value) {
      store.beginWrite()
        .putString(KEY_SOCIAL_TOKEN, value)
        .apply()
    }

  var socialRegistred: Boolean
    get() {
      return getBoolean(KEY_SOCIAL_REGISTRED, false)
    }
    set(value) {
      store.beginWrite()
        .putBoolean(KEY_SOCIAL_REGISTRED, value)
        .apply()
    }
