...
public final class RecipientBottomSheetDialogFragment extends BottomSheetDialogFragment {
  ...
  public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
    ...
    openProfile = view.findViewById(R.id.rbs_open_profile_button);
  ...
  public void onViewCreated(@NonNull View fragmentView, @Nullable Bundle savedInstanceState) {
    ...
    openProfile.setOnClickListener(view -> {
      Intent intent = ProfileActivity.Companion.newIntent(requireActivity(), null, null, Objects.requireNonNull(viewModel.getRecipient().getValue()).getServiceId().get().uuid().toString());
    ...
    viewModel.getRecipient().observe(getViewLifecycleOwner(), recipient -> {
      ...
      avatar.setFallbackPhotoProvider(new Recipient.FallbackPhotoProvider() {
        ...
        public @NonNull FallbackContactPhoto getPhotoForLocalNumber() {
          return new FallbackPhoto80dp(R.drawable.ic_fluent_note_48_regular, recipient.getAvatarColor());
        }
      });
      ...
      if (recipient.isSystemContact() && !recipient.isSelf()) {
        ...
        Drawable drawable = ContextUtils.INSTANCE.getUserBadge(requireContext(), recipient.getBadgeId(getContext()), 28);
        if (drawable!=null){
          SpanUtil.appendCenteredImageSpan(nameBuilder, drawable, 28,28);
        }
      }
