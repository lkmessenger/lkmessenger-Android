...
class KeyboardPageSearchView @JvmOverloads constructor(
  ...
  init {
    ...
    context.obtainStyledAttributes(attrs, R.styleable.KeyboardPageSearchView, 0, 0).use { typedArray ->
      ...
      val backgroundTint = typedArray.getColor(R.styleable.KeyboardPageSearchView_search_bar_tint, ContextCompat.getColor(context, R.color.signal_colorSecondaryBackground))
  ...
  fun enableBackNavigation(enable: Boolean = true) {
    navButton.setImageResource(if (enable) R.drawable.ic_fluent_chevron_left_24_regular else R.drawable.ic_fluent_search_24_regular)
    if (enable) {
      navButton.setImageResource(R.drawable.ic_fluent_chevron_left_24_regular)
      navButton.setOnClickListener { callbacks?.onNavigationClicked() }
    } else {
      navButton.setImageResource(R.drawable.ic_fluent_search_24_regular)
