...
//                letterPaint.setTypeface(AndroidUtilities.getTypeface("fonts/rmedium.ttf"));
                paint2.setColor(ContextCompat.getColor(context, R.color.signal_colorBackground));
                fastScrollBackgroundDrawable = ContextCompat.getDrawable(context, R.drawable.ic_fluent_calendar_day_24_regular).mutate();
                fastScrollBackgroundDrawable.setColorFilter(new PorterDuffColorFilter(ColorUtils.blendARGB(ContextCompat.getColor(context, R.color.signal_colorBackground), Color.WHITE, 0.1f), PorterDuff.Mode.MULTIPLY));
            }
            for (int a = 0; a < 8; a++) {
                radii[a] = AndroidUtilities.dp(44);
            }

            scrollX = AndroidUtilities.dp((type == LETTER_TYPE ? 132 : 240) - 15);
//            updateColors();
...
//        }
        ...
        public boolean onTouchEvent(MotionEvent event) {
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    if (x < AndroidUtilities.dp(107) || lastY < currentY || lastY > currentY + AndroidUtilities.dp(30)) {
                        return false;
                    }
                    if (type == DATE_TYPE && !floatingDateVisible) {
                        if (x < (getMeasuredWidth() - AndroidUtilities.dp(25)) || lastY < currentY || lastY > currentY + AndroidUtilities.dp(30)) {
                            return false;
                        }
                    }
        ...
        protected void onDraw(Canvas canvas) {
            ...
            if (type == LETTER_TYPE) {
                ...
                paint.setColor(ColorUtils.blendARGB(ContextCompat.getColor(getContext(), R.color.signal_colorBackground), Color.WHITE, 0.1f));
                ...
                paint.setColor(ContextCompat.getColor(getContext(), R.color.amber_100));
                canvas.save();
            ...
            if (type == LETTER_TYPE) {
                if ((isMoving || bubbleProgress != 0)) {
                    ...
                    if ((radii[2] != raduisTop || radii[4] != raduisBottom)) {
                        {
                            radii[2] = radii[3] = raduisTop;
                            radii[4] = radii[5] = raduisBottom;
                        }
                        path.reset();
                        rect.set(0, 0, AndroidUtilities.dp(88), AndroidUtilities.dp(88));
...
//        setGlowColor(ContextCompat.getColor(context, R.color.signal_colorBackground));
        selectorDrawable = Theme.getSelectorDrawable(context, ContextCompat.getColor(context, R.color.signal_colorPrimaryContainer), false);
...
//                        FileLog.e(e);
    ...
    public void setSelectorDrawableColor(int color) {
        ...
        if (selectorType == 8) {
            ...
            selectorDrawable = Theme.getSelectorDrawable(getContext(), color, false);
        } else {
